...
class LLMAgent(Agent):
    ...
    def __init__(self,
                 name: str,
                 model: str = "",            # will default to tracking.default_model
                 client: OpenAI|None = None, # will default to tracking.default_client
        ...
        self.name = name 
        self.model = model or tracking.default_model
        self.client = client or tracking.default_client
...
class EvaluationAgent(LLMAgent):
    ...
    party (you).  They also sample from different LLMs and at different temperatures
    (unless these defaults are overridden with keyword args)."""

    # Future improvement: Use constrained decoding.
    # https://platform.openai.com/docs/guides/structured-outputs .
    #
    # That would also allow us to ask several questions in the same
    # call, and have the model fill out a JSON form with both responses
    # and ratings.  It would also ensure that the ratings are integers
    ...
    def __init__(self, 
                 ...
                 model: str = "",           # will default to tracking.default_eval_model
                 **kwargs,
                ) -> None:
        ...
        # Now initialize self.
        model = model or tracking.default_eval_model    # override the LLMAgent default!
    ...
    def rating(self, d: Dialogue,
        ...
        # Future improvement: We might want to generate many responses
        # in parallel (e.g., with temperature=1, n=20), and return
        # their average.  We'd only have to pay for the input tokens once.
        # We'd have to pay for the output tokens of all n responses,
        # but the outputs are very short.
...
# 
# Warning: evaluate.eval_on_characters creates its own CharacterAgents 
